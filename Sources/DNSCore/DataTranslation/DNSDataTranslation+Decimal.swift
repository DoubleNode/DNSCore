...
public extension DNSDataTranslation {
    // MARK: - decimal...
    func decimal<K>(from container: KeyedDecodingContainer<K>,
                    forKey key: KeyedDecodingContainer<K>.Key) -> Decimal? where K: CodingKey {
        do { return decimal(from: try container.decodeIfPresent(Decimal.self, forKey: key)) } catch { }
        do { return decimal(from: try container.decodeIfPresent(Date.self, forKey: key)) } catch { }
        do { return decimal(from: try container.decodeIfPresent(URL.self, forKey: key)) } catch { }
        do { return decimal(from: try container.decodeIfPresent(Double.self, forKey: key)) } catch { }
        do { return decimal(from: try container.decodeIfPresent(Float.self, forKey: key)) } catch { }
        do { return decimal(from: try container.decodeIfPresent(UInt.self, forKey: key)) } catch { }
        do { return decimal(from: try container.decodeIfPresent(Int.self, forKey: key)) } catch { }
        do { return decimal(from: try container.decodeIfPresent(String.self, forKey: key)) } catch { }
        do { return decimal(from: try container.decodeIfPresent(Bool.self, forKey: key)) } catch { }
        return nil
    }
